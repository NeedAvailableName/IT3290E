...
--6
select Company.Name, COUNT(Supply.ProductID)
from Company, Supply
where Company.CompanyID = Supply.CompanyID
group by Company.Name
having count(Supply.ProductID) >= 2
 ...
 where CompanyID in (
	 ...
	 where Color = 'blue'
	 group by CompanyID
	 having COUNT(*) = (select COUNT(*) from Product where Color = 'blue')
)

 --8
 select top 1 Company.CompanyID, Company.Name
 from Company, Supply
 where Company.CompanyID = Supply.CompanyID
 group by Company.CompanyID, Name
 order by COUNT(*) desc

 --find company only provide brown items
 ...
 --9
 select Company.CompanyID, Company.Name, AVG(Supply.ProductID)
 from Company, Supply
 where Company.CompanyID = Supply.CompanyID
 group by Company.CompanyID, Name
 order by AVG(Supply.ProductID) desc
